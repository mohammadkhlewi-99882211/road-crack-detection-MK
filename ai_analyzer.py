...
def _merge_detections(all_detections):
    """
    دمج نتائج عدة نماذج وحذف التكرار بناءً على IoU.
    الشرخ الذي يكتشفه أكثر من نموذج يحصل على أولوية أعلى.
    """
    merged = []
    for det in all_detections:
        ...
        for m in merged:
            if _iou(det["bbox"], m["bbox"]) > IOU_THRESHOLD:
                # دمج: احتفظ بالأعلى ثقة وزِد عدد النماذج التي كشفته
...
# ─────────────────────────────────────────────
#  STEP 1 — Roboflow: كشف من كل نموذج
# ─────────────────────────────────────────────
