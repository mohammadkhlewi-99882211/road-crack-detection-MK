import os
import json
import base64
import re
import io
import google.generativeai as genai
from PIL import Image


# ─────────────────────────────────────────────
#  Client
# ─────────────────────────────────────────────

def _get_model(temperature=0.1):
    api_key = os.environ.get("GEMINI_API_KEY")
    if not api_key:
        raise ValueError("GEMINI_API_KEY is not set")
    genai.configure(api_key=api_key)
    return genai.GenerativeModel(
        model_name="gemini-1.5-flash",   # flash أسرع وأكثر استقراراً من pro على Streamlit Cloud
        generation_config=genai.GenerationConfig(
            max_output_tokens=4096,
            temperature=temperature,
        )
    )


# ─────────────────────────────────────────────
#  JSON parser
# ─────────────────────────────────────────────

def _parse_json(text):
    if not text:
        return None
    # إزالة markdown fences
    text = re.sub(r"```(?:json)?", "", text).strip()
    # محاولة 1: parse مباشر
    try:
        return json.loads(text)
    except Exception:
        pass
    # محاولة 2: استخراج أول { ... }
    m = re.search(r"\{.*\}", text, re.DOTALL)
    if m:
        try:
            return json.loads(m.group())
        except Exception:
            pass
    # محاولة 3: إصلاح الفواصل الزائدة
    if m:
        fixed = re.sub(r",\s*([}\]])", r"\1", m.group())
        try:
            return json.loads(fixed)
        except Exception:
            pass
    return None


# ─────────────────────────────────────────────
#  تحويل الصورة
# ─────────────────────────────────────────────

def _b64_to_pil(image_base64):
    data = base64.b64decode(image_base64)
    img = Image.open(io.BytesIO(data)).convert("RGB")
    # تصغير الصورة إذا كانت كبيرة جداً لتجنب timeout
    max_px = 1024
    w, h = img.size
    if max(w, h) > max_px:
        scale = max_px / max(w, h)
        img = img.resize((int(w * scale), int(h * scale)), Image.LANCZOS)
    return img


# ─────────────────────────────────────────────
#  Prompts
# ─────────────────────────────────────────────

DETECT_PROMPT = """Analyze this image and detect ALL cracks, fractures, and surface defects.

You MUST respond with ONLY a JSON object. No text before or after. No markdown.

JSON format:
{"detected":[{"id":1,"bbox":{"x":0.1,"y":0.2,"width":0.3,"height":0.1},"confidence":85,"rough_type":"linear crack","rough_severity":"HIGH"}],"total":1,"image_quality":"good","surface_visible":"concrete"}

Rules for bbox (all values 0.0 to 1.0):
- x = left edge of crack
- y = top edge of crack  
- width = horizontal size
- height = vertical size

If no cracks found: {"detected":[],"total":0,"image_quality":"good","surface_visible":"unknown"}

Respond with JSON only:"""

ANALYZE_PROMPT = """You are a structural engineer. Analyze these pre-detected cracks.

Crack locations:
{boxes}

Image: {w}x{h}px

Respond with ONLY this JSON (Arabic text, no markdown):
{{"summary":"ملخص الحالة","overall_severity":"HIGH","overall_confidence":85,"material_type":"خرسانة","surface_condition":"وصف السطح","environmental_factors":"العوامل البيئية","cracks":[{{"id":1,"bbox":{{"x":0.1,"y":0.2,"width":0.3,"height":0.1}},"type":"شرخ طولي","category":"structural","is_structural":true,"estimated_width_mm":"1-2","estimated_length_cm":"20-30","depth_assessment":"متوسط","severity":"HIGH","confidence":85,"description":"وصف","cause_analysis":"السبب","progression_risk":"متوسط","immediate_action":"الإجراء"}}],"recommendations":[{{"priority":1,"action":"الإجراء","timeline":"أسبوع","estimated_cost_level":"متوسط","details":"التفاصيل"}}],"monitoring_plan":"خطة المتابعة","professional_consultation_required":true,"notes":""}}

Use EXACT bbox values from the crack locations above. overall_severity must be: CRITICAL, HIGH, MEDIUM, or LOW.
JSON only:"""


# ─────────────────────────────────────────────
#  الكشف عن الشقوق
# ─────────────────────────────────────────────

def _detect(image_base64):
    pil_img = _b64_to_pil(image_base64)
    
    for attempt in range(3):
        try:
            model = _get_model(temperature=round(0.1 + attempt * 0.15, 2))
            response = model.generate_content([DETECT_PROMPT, pil_img])
            
            raw = response.text if response and response.text else ""
            print(f"[DETECT attempt {attempt+1}] response: {raw[:400]}")
            
            result = _parse_json(raw)
            if result and isinstance(result.get("detected"), list):
                print(f"[DETECT] success: {len(result['detected'])} cracks found")
                return result
            else:
                print(f"[DETECT attempt {attempt+1}] parse failed or no 'detected' key")
                
        except Exception as e:
            print(f"[DETECT attempt {attempt+1}] exception: {e}")
    
    print("[DETECT] all attempts failed")
    return {"detected": [], "total": 0}


# ─────────────────────────────────────────────
#  التحليل الهندسي
# ─────────────────────────────────────────────

def _analyze(image_base64, detections, img_w, img_h):
    pil_img = _b64_to_pil(image_base64)
    
    boxes_text = ""
    for d in detections:
        b = d.get("bbox", {})
        boxes_text += (f"Crack #{d['id']}: x={b.get('x',0):.3f} y={b.get('y',0):.3f} "
                       f"w={b.get('width',0):.3f} h={b.get('height',0):.3f} | "
                       f"type={d.get('rough_type','crack')} severity={d.get('rough_severity','MEDIUM')}\n")
    
    prompt = ANALYZE_PROMPT.format(boxes=boxes_text, w=img_w, h=img_h)
    
    for attempt in range(2):
        try:
            model = _get_model(temperature=round(0.2 + attempt * 0.1, 2))
            response = model.generate_content([prompt, pil_img])
            raw = response.text if response and response.text else ""
            print(f"[ANALYZE attempt {attempt+1}] response: {raw[:300]}")
            result = _parse_json(raw)
            if result and "summary" in result:
                return result
        except Exception as e:
            print(f"[ANALYZE attempt {attempt+1}] exception: {e}")
    
    # Fallback بسيط
    sev_priority = ["CRITICAL", "HIGH", "MEDIUM", "LOW"]
    all_sevs = [d.get("rough_severity", "MEDIUM") for d in detections]
    overall = next((s for s in sev_priority if s in all_sevs), "MEDIUM")
    
    return {
        "summary": f"تم اكتشاف {len(detections)} شرخ في السطح. يُنصح بالفحص الميداني.",
        "overall_severity": overall,
        "overall_confidence": 70,
        "material_type": "غير محدد",
        "surface_condition": "يوجد شروخ تستوجب الفحص",
        "environmental_factors": "",
        "cracks": [
            {
                "id": d["id"], "bbox": d["bbox"],
                "type": d.get("rough_type", "شرخ"),
                "category": "structural", "is_structural": True,
                "estimated_width_mm": "غير محدد", "estimated_length_cm": "غير محدد",
                "depth_assessment": "غير محدد",
                "severity": d.get("rough_severity", "MEDIUM"),
                "confidence": int(d.get("_conf", 0.7) * 100),
                "description": "تم الكشف بواسطة نظام الذكاء الاصطناعي",
                "cause_analysis": "يتطلب فحصاً ميدانياً",
                "progression_risk": "متوسط",
                "immediate_action": "فحص ميداني"
            }
            for d in detections
        ],
        "recommendations": [{"priority": 1, "action": "فحص ميداني عاجل",
                              "timeline": "خلال أسبوع", "estimated_cost_level": "متوسط",
                              "details": "مراجعة مهندس إنشائي لتقييم الشروخ المكتشفة"}],
        "monitoring_plan": "متابعة الشروخ شهرياً",
        "professional_consultation_required": True,
        "notes": ""
    }


# ─────────────────────────────────────────────
#  الدالة الرئيسية
# ─────────────────────────────────────────────

def detect_and_analyze(image_base64, img_width, img_height):
    # الكشف
    raw_result = _detect(image_base64)
    raw_boxes  = raw_result.get("detected", [])

    # توحيد صيغة bbox
    for b in raw_boxes:
        if "bbox" not in b:
            b["bbox"] = {
                "x": b.pop("x", 0), "y": b.pop("y", 0),
                "width": b.pop("width", 0.05), "height": b.pop("height", 0.05)
            }
        b["_conf"] = float(b.get("confidence", 75)) / 100.0

    # تصحيح القيم وإعادة الترقيم
    final = []
    for i, det in enumerate(raw_boxes):
        bx = det["bbox"]
        x = max(0.0, min(0.98, float(bx.get("x", 0))))
        y = max(0.0, min(0.98, float(bx.get("y", 0))))
        w = max(0.02, min(1.0 - x, float(bx.get("width", 0.05))))
        h = max(0.02, min(1.0 - y, float(bx.get("height", 0.05))))
        det["bbox"] = {"x": x, "y": y, "width": w, "height": h}
        det["id"] = i + 1
        final.append(det)

    total = len(final)
    print(f"[PIPELINE] final detections: {total}")

    # لا شقوق
    if total == 0:
        return {
            "total_cracks_detected": 0,
            "summary": "لم يتم الكشف عن أي شروخ أو شقوق. السطح يبدو بحالة جيدة.",
            "overall_severity": "LOW", "overall_confidence": 90,
            "material_type": raw_result.get("surface_visible", "غير محدد"),
            "surface_condition": "السطح في حالة جيدة",
            "environmental_factors": "", "cracks": [],
            "recommendations": [{"priority": 1, "action": "الصيانة الوقائية الدورية",
                                  "timeline": "كل 6-12 شهراً", "estimated_cost_level": "منخفض",
                                  "details": "فحص دوري للحفاظ على الحالة الجيدة"}],
            "monitoring_plan": "فحص بصري كل 6 أشهر",
            "professional_consultation_required": False, "notes": "",
            "_detection_info": {"gemini_detected": 0}
        }

    # التحليل
    analysis = _analyze(image_base64, final, img_width, img_height)
    analysis["total_cracks_detected"] = total
    analysis["_detection_info"] = {"gemini_detected": total}

    # مزامنة الـ bbox
    if analysis.get("cracks"):
        for i, crack in enumerate(analysis["cracks"]):
            if i < len(final):
                crack["bbox"] = final[i]["bbox"]
            crack["id"] = i + 1
    else:
        analysis["cracks"] = [
            {
                "id": d["id"], "bbox": d["bbox"],
                "type": d.get("rough_type", "شرخ"), "category": "structural",
                "is_structural": True, "estimated_width_mm": "غير محدد",
                "estimated_length_cm": "غير محدد", "depth_assessment": "غير محدد",
                "severity": d.get("rough_severity", "MEDIUM"),
                "confidence": int(d.get("_conf", 0.7) * 100),
                "description": "تم الكشف بواسطة نظام الذكاء الاصطناعي",
                "cause_analysis": "يتطلب فحصاً ميدانياً",
                "progression_risk": "متوسط", "immediate_action": "فحص ميداني"
            }
            for d in final
        ]

    return analysis


# ─────────────────────────────────────────────
#  توصيات الداشبورد
# ─────────────────────────────────────────────

def generate_dashboard_recommendations(records_summary):
    prompt = f"""Based on crack analysis records below, give maintenance recommendations in Arabic.
Return ONLY JSON, no markdown:
{{"overall_assessment":"تقييم شامل","priority_actions":["إجراء 1","إجراء 2"],"maintenance_schedule":"الجدول","budget_estimate":"متوسط","risk_areas":["منطقة 1"],"preventive_measures":["إجراء وقائي"]}}

Records:
{records_summary}

JSON only:"""
    try:
        model = _get_model(temperature=0.3)
        response = model.generate_content([prompt])
        result = _parse_json(response.text or "")
        if result:
            return result
    except Exception as e:
        print(f"[DASHBOARD] error: {e}")
    return {"overall_assessment": "تعذّر توليد التوصيات، يرجى المحاولة مرة أخرى."}
